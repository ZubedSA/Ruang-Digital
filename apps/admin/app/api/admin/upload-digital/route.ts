...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      ...
      try {
        ...
        const gasRes = await fetch(gasUrl, {
          ...
          body: JSON.stringify({
            ...
            base64Data,
            isPublic: false, // File digital bersifat PRIVAT (hanya pemilik & pembeli terverifikasi)
          }),
        });
      ...
      // Jika URL GAS belum disiapkan atau masih placeholder
      errorMessage = 'GOOGLE_APPS_SCRIPT_URL masih default / belum dihubungkan.';
    }

    // Fallback: Jika GAS belum diatur atau gagal terhubung, simpan ke local secure storage
    const baseDir = process.cwd();
    const localDigitalDir = path.resolve(baseDir, '..', 'web', 'storage', 'digital');
    ...
    // Jika berhasil ke Google Drive, gunakan fileId dari Drive. Jika tidak, gunakan ID lokal
    const finalFileId = driveFileId || `local_${localSavedFileName}`;
    ...
    return NextResponse.json({
      ...
      message:
        storageType === 'google-drive'
          ...
          : `File disimpan di server lokal (${errorMessage || 'Local fallback'}).`,
    });
  } catch (error: any) {
