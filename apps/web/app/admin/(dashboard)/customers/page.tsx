...
export default async function AdminCustomersPage() {
  ...
  const activeBuyersCount = customers.filter((c: any) => c.orders.length > 0).length;
  const totalCustomerSpending = customers.reduce((acc: number, c: any) => {
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2.5">
          <Users className="h-5 w-5 md:h-6 md:w-6 text-purple-600 dark:purple-400" />
          Direktori Pelanggan
        </h1>
